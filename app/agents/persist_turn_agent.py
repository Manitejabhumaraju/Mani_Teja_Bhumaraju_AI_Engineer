"""
PersistTurnAgent: agent wrapper around the persist_turn node factory.
...
from app.graph.nodes import make_persist_turn_node
from app.graph.state import GraphState


class PersistTurnAgent(BaseAgent):
    name = "persist_turn"

    def __init__(self, deps: AgentDeps):
        super().__init__(deps)
        # Build the underlying node closure once; reuse on every call.
        self._fn = make_persist_turn_node(deps)
