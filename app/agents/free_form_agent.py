"""
FreeFormAgent: agent wrapper around the free_form node factory.
...
from app.graph.nodes import make_free_form_node
from app.graph.state import GraphState


class FreeFormAgent(BaseAgent):
    name = "free_form"

    def __init__(self, deps: AgentDeps):
        super().__init__(deps)
        # Build the underlying node closure once; reuse on every call.
        self._fn = make_free_form_node(deps)
