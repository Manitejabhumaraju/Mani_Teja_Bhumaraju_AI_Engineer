"""
CityRCAAgent: agent wrapper around the city_rca node factory.
...
from app.graph.nodes import make_city_rca_node
from app.graph.state import GraphState


class CityRCAAgent(BaseAgent):
    name = "city_rca"

    def __init__(self, deps: AgentDeps):
        super().__init__(deps)
        # Build the underlying node closure once; reuse on every call.
        self._fn = make_city_rca_node(deps)
